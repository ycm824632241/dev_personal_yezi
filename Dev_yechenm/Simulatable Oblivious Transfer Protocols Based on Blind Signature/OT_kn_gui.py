...
class OT_GUI(QWidget):
    ...
    def _on_gdh_init_finished(self, gdh_instance):
        ...
        self.ot_protocol = OTk1(self.gdh)  # <-- 切换到 k-out-of-n 协议
    ...
    def _create_receiver_group(self):
        ...
        self.sigma_entry = QLineEdit("2, 3") # <-- 默认选择多个
        layout.addRow(QLabel("选择秘密的索引 (σ):"), self.sigma_entry)

        hint_label = QLabel("提示：可输入多个索引，用逗号分隔。")
        hint_label.setStyleSheet("color: gray; font-style: italic;")
        layout.addRow("", hint_label)
    ...
    def run_receiver_query(self):
        self.log("\n" + "="*20 + " 步骤 2: 接收方生成查询 " + "="*20)
        try:
            sigma_str = self.sigma_entry.text()
            sigma_list = [int(s.strip()) for s in sigma_str.split(',') if s.strip()]
            if not sigma_list:
                self.log("❌ 错误: 请至少输入一个有效的索引。")
                return

            for sigma in sigma_list:
                if not (1 <= sigma <= len(self.secrets)):
                    self.log(f"❌ 错误: σ 的值 {sigma} 必须在 1 到 {len(self.secrets)} 之间。")
                    return
        except ValueError:
            self.log("❌ 错误: σ 必须是一个或多个用逗号分隔的整数。")
            return

        self.log(f"接收方选择获取第 {sigma_list} 个秘密。")
        try:
            # 调用自适应查询方法
            self.query_data = self.ot_protocol.receiver_adaptive_query(sigma_list, self.public_params)
            self.log(f"接收方已对 {len(sigma_list)} 个选择的秘密标识进行盲化。")
            for i, (blind_msg, r, sigma) in enumerate(self.query_data):
                self.log(f"  - 查询 {i+1} (for σ={sigma}):")
    ...
    def run_sender_response(self):
        ...
        self.log(f"发送方收到了 {len(self.query_data)} 个盲消息，开始生成响应。")
        try:
            # 调用自适应响应方法
            self.response_data = self.ot_protocol.sender_adaptive_response(self.query_data, self.private_params, self.public_params)
            self.log("发送方已对每个盲消息签名并生成了加密的AES密钥。")
            for i, (Y_sigma, C_j_list) in enumerate(self.response_data):
                 self.log(f"  - 响应 {i+1}:")
                 self.log(f"    - Y_σ (前32位): {hex(Y_sigma)[:32]}...")
                 self.log(f"    - 生成了 {len(C_j_list)} 个加密的AES密钥 C_j。")
    ...
    def run_receiver_decrypt(self):
        ...
        self.log(f"接收方使用自己的盲化因子和收到的响应来解密 {len(self.query_data)} 个秘密。")
        try:
            # 调用自适应解密方法
            decrypted_secrets = self.ot_protocol.receiver_adaptive_decrypt(self.response_data, self.query_data, self.public_params)
            self.log(f"🎉 解密成功！")

            validation_ok = True
            for i, decrypted_secret in enumerate(decrypted_secrets):
                sigma = self.query_data[i][2]
                expected_secret = self.secrets[sigma - 1]
                self.log(f"  - 第 {sigma} 个秘密:")
